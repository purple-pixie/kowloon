...
func get_selectable_dice(values, selected = []):
	## get all dice that could form part of a meld
	if len(selected) > 1:
		if selected[1] == selected[0]:
			return {selected[0]: null}
	if not len(selected):
		return find_biggest_hand(values)
	var run = find_biggest_run(values)
	if len(selected) == 1:
		if selected[0] in run:
			return run
		else:
			return {selected[0]: null}
	var faces = {}
	...
	for melds in best_melds:
		...
		for meld in melds:
			if not meld is Array:
				print_debug("find_biggest_hand expected Array found:")
				print_debug(values)
				print_debug(melds)
